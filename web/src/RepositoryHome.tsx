...
type PageRequest = { query: string; cursor: string | null; previous: (string | null)[]; revision: number };
...
export function RepositoryHome({ session, auth, navigate }: { session: Session; auth: SessionController; navigate: (path: string) => void }) {
  ...
  const [request, setRequest] = useState<PageRequest>({ query: "", cursor: null, previous: [], revision: 0 });
  ...
  useEffect(() => {
    ...
    void readRepositories({ query: request.query, cursor: request.cursor, signal: controller.signal }).then((response) => {
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    setRequest((before) => ({ query: search, cursor: null, previous: [], revision: before.revision + 1 }));
  }

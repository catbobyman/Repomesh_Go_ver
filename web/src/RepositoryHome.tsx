...
type PageRequest = { query: string; cursor: string | null; previous: (string | null)[]; revision: number; refresh: boolean };
...
export function RepositoryHome({ session, auth, navigate }: { session: Session; auth: SessionController; navigate: (path: string) => void }) {
  ...
  const [request, setRequest] = useState<PageRequest>({ query: "", cursor: null, previous: [], revision: 0, refresh: false });
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    setRequest((before) => ({ query: search, cursor: null, previous: [], revision: before.revision + 1, refresh: false }));
  }

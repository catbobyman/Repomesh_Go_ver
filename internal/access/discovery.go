...
func (s *Service) Repositories(ctx context.Context, cookie string, query RepositoryQuery) (RepositoryPage, error) {
	...
	if b.id == "" {
		err = tx.QueryRow(ctx, `SELECT id FROM repomesh_access.discovery_batches WHERE actor=$1 AND connection_revision=$2 AND access_epoch=$3 AND query=$4 AND (NOT complete OR NOT delivered) AND expires_at>now() ORDER BY expires_at DESC LIMIT 1`, view.User.ID, revision, epoch, query.Text).Scan(&b.id)
		if errors.Is(err, pgx.ErrNoRows) {
			b.id = newID()

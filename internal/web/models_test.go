...
func TestPostgresModelSaveLostResponseAndRestart(t *testing.T) {
	...
	restarted.Config.Handler = server.wrapProduct(handlerConfigured(os.DirFS(assets), Auth{Service: server.auth, Origin: restartedOrigin}, Projects{Service: projectService}, Models{Service: modelService}, Scan{}, Decision{}, Skills{}))
	restarted.StartTLS()

#!/usr/bin/env python3
"""Prepare/start only rv-a/rv-b AgentTeams validation instances.
...
RUN_ID = "agentteams-2026-09-09"
OWNER_KEY = "repomesh.validation.run"
INSTANCE_KEY = "repomesh.validation.instance"
IMAGE = "repomesh-validation/embedded:eeaab643"
MANAGER_IMAGE = "repomesh-validation/manager-qwenpaw:eeaab643"
...
def plan(root, instance):
    base = 28000 if instance == "a" else 38000
    name = f"rv-{instance}"
...
def main():
    ...
    parser.add_argument("--instances", default="a,b", help="a, b, or a,b")
    parser.add_argument("--with-provider", action="store_true", help="inject existing private/provider.json; Manager stays disabled")
    ...
    if not instances or len(set(instances)) != len(instances) or any(value not in {"a", "b"} for value in instances):
        raise DeploymentError("--instances must be a, b, or a,b")
    if not 0 <= options.wait_seconds <= 600:
        raise DeploymentError("--wait-seconds must be between 0 and 600")
    root = Path(__file__).resolve().parents[1]
    configurations = [plan(root, instance) for instance in instances]
    if not options.apply:
        print(json.dumps({"mode": "READ_ONLY_PLAN", "manager_enabled": False,
                          "with_provider": options.with_provider, "instances": configurations}, indent=2))
        return 0
    ...
    provider = load_provider(root, options.with_provider)
    results = []

#!/usr/bin/env python3
"""Prepare/start only rv-d AgentTeams validation instances.
...
def plan(root, instance):
    if instance != "d":
        raise DeploymentError("Only isolated instance d is permitted")
    base = 49000
    name = f"rv-{instance}"
...
def main():
    ...
    parser.add_argument("--instances", default="d", choices=["d"], help="only isolated instance d")
    ...
    if instances != ["d"]:
        raise DeploymentError("--instances must be d")
    ...
    if not options.apply:
        ...
        resources = {name: inspect(kind, name) is not None for kind, name in
                     [("container", "rv-d-controller"), ("volume", "rv-d-data"),
                      ("volume", "rv-d-agentfs"), ("network", "rv-d-net")]}
    ...
    for configuration in configurations:
        ...
        write_json(root / "evidence" / "deployment-d-instance-inventory.json", {

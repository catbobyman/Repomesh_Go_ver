...
| B05 | TODO；范围已缩减 | 运行政策、预算与时限只读摘要、专用模型应用；独立模型测试延期 | B04；按缩减后的 C05/C06 与政策协议重新采用 | 无状态预览不落库；快照隔离；只换模型保留 execution；无变化保存不造修订；受限原模型预览不泄露 |
| B06 | TODO | 手动 Issue 原子创建、必要会话、主 ChangeSet、来源、范围与持久待办 | B05 固定 execution 政策；采用并落实 P9 固定配置绑定。不依赖模型测试或 B10 额度窗口 | DB01 至 DB08 和 CB01 至 CB05；任一写入失败全回滚；创建与换配置并发绑定完整版本；不伪造运行接入 |
...
C05 项目预算/时限摘要和 C06 原模型受限/历史定位已按缩减后的 B05 重述，尚待采用及真实验收。模型应用预览改为无状态比较；运行额度在 B10 账本落地前明确 unavailable。历史文档的后端 B01 至 B08 编号与本表的施工批次编号不同。
...
| [x] | B05，历史候选设计完成；当前范围已缩减，待重新采用后实现 | [当前设计](../api-database/b05.md)只保留运行政策、专用模型应用及 C05／C06；原单次模型测试与配套存储延期，历史设计与复核保留为演进记录。 |
...
B05 实施前须按缩减协议重新采用运行政策、专用模型应用、C05 与 C06，并核对已实现 B04 的接口与版本语义；旧 D05/D06 模型测试协议不自动进入实施。B06 只依赖 B05 的固定 execution 政策，并须采用和落实 P9 配置绑定，不等待模型测试或 B10 额度窗口。每项任务按实际范围确定文件编辑归属和验证方法，不沿用旧轮次的协作名单。

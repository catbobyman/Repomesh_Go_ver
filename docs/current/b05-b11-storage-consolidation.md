...
更新：2026-09-15。原合表基线记录 B05-B11 的 63 张历史候选并压到 34 张。用户随后决定缩减 B05：9 条模型测试专用提案延期，预算窗口移交 B10，当前 54 条活动旧映射压到 27 张目标表，再加 3 张必需新表，共 30 张。本文记录当前范围、延期去向、批次、声明格式和计数口径。

**状态：design_only。** B01-B04 的现有表和已采用逻辑契约不变。B05 模型测试从当前范围延期，不等于已实现或由 AgentTeams 自动接管；运维 `agt llm-preflight` 也不是产品验收。其余 B05-B11 候选、提案、未实施和验收状态不因本次调整升级。清单和校验是静态设计事实，不是数据库行为验证。
...
- 手册外决策链表 3 张：`public.decision_chain_nodes`、`public.decision_embeddings`、`public.feature_settings`，来自 `0008_decision_chain.sql`（origin/main `6899665`）；单独成组，不计入 B05-B11 当前设计。
- 历史候选 63 条中 9 条延期，54 条活动映射压到 27 张目标物理表；另有 `policy_bindings`、`policy_imports` 和 `business_plan_versions` 三张必需新表，当前设计 30 张。
- 手册范围合计 66 = 36 + 30；全仓含扩展合计 70 = 36 + 1 扫描 + 3 决策链 + 30 设计。

## 批次与目标表

| 批次 | 旧表 | 目标 | 声明章节 |
| --- | --- | --- | --- |
| B05 | 16（9 延期） | 4 | `b05.md` |
| B06 | 13 | 6 | `b06.md` |
| B07 | 1 | 1 | `b07.md` |
| B08 | 0 | 0 | 不声明 |
| B09 | 13 | 6 | `b09.md` |
| B10 | 13（另接收 B05 窗口） | 11 | `b10.md` |
| B11 | 7 | 2 | `b11.md` |
...
| `repomesh_sources.policy_bindings` | execution 政策补充；`test_bindings` 随测试延期，不再形成 test 分支 |
| `repomesh_sources.policy_imports` | 新增；schema2 导入原操作。旧 `repomesh_sources.imports` 保持 `schema_version=1` |

延期的九条旧提案是 `models.previews`、`models.tests`、`modelbudget.test_reservations`、`models.test_dispatch`、`models.test_observations`、`models.test_handler_leases`、`models.actor_outstanding_tests`、`models.unknown_test_closures` 和 `test_bindings`。它们保留在清单中用于审计历史覆盖，但 `disposition=deferred`，没有当前 target。`modelbudget.windows` 不延期，改由 B10 的 `repomesh_modelbudget.windows` 承载真实项目运行额度。当前 `policy_bindings` 是 execution 政策补充所需的新承载，不再宣称由 test binding 合并而来。
...
B10/B11 的旧 20 张执行提案先合到 11 张承载，再补 1 张业务计划版本，共 12 张执行域表；另接收 B05 的项目运行额度窗口，B10/B11 批次合计 13 张。B05-B11 当前设计合计 30 张。

| 目标物理表 | 旧表承载 |
| --- | --- |
| `repomesh_modelbudget.windows` | B05 历史 `modelbudget.windows`；只保留 `project_model_runtime` 项目运行窗口 |
...
- 该表的来源是原 B10/B11 引用而未建模的已采用业务 Plan Version（ADR-0003、ADR-0015、Graph §5），不从原 63 张旧表映射里虚构旧表名。额度窗口移交后 B10 从原 10 张调整为 11 张。

## 声明与机器清单

完整的 63 条历史映射、活动／延期状态、批次归属和快照来源在 [table-manifest.json](../api-database/table-manifest.json)。每张当前目标物理表只在所属批次正文声明一次，独立一行：
...
- 不把 B05-B11 的候选、提案或未实施状态升级为已采用或已实现；延期也不等于删除历史设计或已由上游能力验收。

...
| B07—B11 | 后续实施 TODO | Issue 查询、管理闭环及真实运行接入未完成。B09 的四项数据兼容问题仅有静态部分覆盖，完整 G1、G2 未完成。[F07—F15 假数据工作区](f07-f15-workspace-browser-demo.md) 是 Vite 演示（含独立 `web/fake-backend/`、可进房间与只读原生 DAG），不能当作本批产品已实现。 |

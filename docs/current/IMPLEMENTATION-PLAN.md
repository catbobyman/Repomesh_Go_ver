...
| B04 | DESIGN_ADOPTED；IMPLEMENTED；INTEGRATED_LOCAL_VERIFIED；授权范围已结束，非整批 VERIFIED | 模型供应商保存／close、不可变版本、部署 schema 1 导入、六个 HTTP 端点与 Key 恢复。不发送模型请求 | 用户授权推进 B04；采用记录见 [b04-model-sources-adoption.md](b04-model-sources-adoption.md) | [验收报告](../development/2026-09-13-b04-acceptance-01/README.md)：`main` `621592d`，260／0／2、前端 32／32、S01—S12、U04.2／U04.4 夹具。真实模型与 GitHub 未跑，businessReady=false |

...
| B04 | DESIGN_ADOPTED；IMPLEMENTED；INTEGRATED_LOCAL_VERIFIED，非整批 VERIFIED | 模型供应商保存／close、不可变版本、部署 schema 1 导入、六个 HTTP 端点与 Key 恢复。不发送模型请求 | 用户授权推进 B04；采用记录见 [b04-model-sources-adoption.md](b04-model-sources-adoption.md) | 收口 01：259／0／2、race、前端 31／31、B03 夹具 25+1、B04 Key 恢复夹具、当时独立复核无开放 P0／P1／P2、包 `0.4.0-b04-integrated-20260913-r2`，businessReady=false。收口后另一次复核发现 P1／P2，由后续修复跟进。真实模型与 GitHub 未跑 |

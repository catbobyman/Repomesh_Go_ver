...
> **范围与协议替代说明（2026-09-15）：** 当前 B05 以 [缩减后的设计](../api-database/b05.md) 为准：M04—M06 的模型测试、handler、SendPermit、unknown 关闭和测试观察整体延期；M07 模型应用保留，但应用预览改为无状态比较，不再持久 `previewId`。项目运行额度窗口移交 B10。本文测试章节和旧预览消费协议只保留历史候选，不再是当前实施前置；B01-B04 已实现逻辑仍以原采用记录为准。

...
| 范围 | 设计入口与开工边界 |
| --- | --- |
| B01 身份／权限／模型配置 | [身份内部候选](backend-first-development-access-draft.md)、[认证浏览器候选](authentication-browser-api-draft.md)、[来源候选](backend-first-batch-sources-draft.md)、[模型内部候选](backend-model-operations-draft.md)。owner、刷新互斥、密钥只写、版本、费用与出站职责仍须按候选范围采用并实现。 |
...
| B04 浏览器用例 | [首批契约](first-batch-browser-api-contract.md)、[创建契约](issue-page-create-api-contract.md)、[页面恢复](first-batch-recovery-design.md)。按唯一字段和权限规则交付；首批新增认证／模型协议仍是候选。 |
...
数据库连接、显式迁移和核查已实现，见[数据库说明](database-development.md)和[施工批次表](IMPLEMENTATION-PLAN.md)。当前业务表、认证、秘密存储、外部投递及执行器仍未实现。[审查修订](design-readiness-revisions.md)补齐的是候选和文档语义，真实故障验收按开发指南和各专题进行。旧归档未运行 Go/npm、数据库或历史上游实验；本次数据库施工的实际验收见[B01 记录](../development/2026-09-12-batch-01/README.md)。

...
更新：2026-09-09，持续设计授权与双方创建基线同步完成后的交接版。用于接续后端架构设计；页面／接口侧继续使用 [NEXT-SESSION-PROMPT.md](NEXT-SESSION-PROMPT.md)。通信按最新[通信约定](design-communication.md)执行。复制下面整个文本块到新会话，或要求新会话完整读取本文件后执行。本文件不创建任务，也不启动实现。
...
ADR-0014：Graph 插件在后台协调进程内按独立模块运行，随 Go 后端发布；计算流程，后台核验后实际派工；计划、轮次和累计消耗持久化。接受共同升级和严重插件故障影响后台的代价，具体插件接口／机制未选定。
ADR-0015：当前获准轮次映射成有限 DAG，RepoMesh 控制后续轮次；既定范围和上限内继续不产生新的业务 Plan Version。不依赖 Controller replan 已能重写完整原生 Loop；需 replan 时按完整受影响上游 Project 收敛、核对 submitted、应用及读回。
...
AgentTeams 依据锁定提交 eeaab64391ccaec9118e84977f538aefd40720d6 的静态调研，未部署验收。Controller REST 提供资源生命周期及部分工作流能力；taskflow 委派／提交等主要是本地 stdio MCP，不能虚构 REST 任务启动端点。
上游 Project 身份涉及实例、team、project_id。replan 遇到 in-progress／submitted 拒绝，pause 不停止在途。原生 DAG／Loop 与 Dashboard HITL 不证明 RepoMesh 全部编排和审批已实现。

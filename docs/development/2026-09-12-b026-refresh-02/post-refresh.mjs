  ...
  const inspectRequest = request => {
    try {
      ...
      if (
        ...
        ['/api/auth/github/login', '/api/auth/github/reconnect'].includes(url.pathname)
      ) {
        forbiddenPost = true;
        writeEvent('history-setup-forbidden-post', { path: url.pathname, status: 0 });
      }
    } catch { /* Ignore unparseable third-party request URLs. */ }
  };
  page.on('request', inspectRequest);
  try {
    await page.getByRole('button', { name: '重新连接 GitHub', exact: true }).click();
    await page.waitForURL(url => url.origin === allowedOrigin && url.pathname === '/login', { timeout: 15_000 });
    if (forbiddenPost) fail('AUTHORIZATION_POST_DURING_HISTORY_SETUP');
    writeEvent('history-setup', { origin: allowedOrigin, path: '/login' });

    const returnedHome = page.waitForURL(url => url.origin === allowedOrigin && url.pathname === '/', { timeout: 15_000 });
    await page.getByRole('button', { name: '稍后处理，返回工作区', exact: true }).click();
    await returnedHome;
    await page.getByRole('button', { name: '退出登录', exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
    if (forbiddenPost) fail('AUTHORIZATION_POST_DURING_HISTORY_SETUP');
    writeEvent('history-setup', { origin: allowedOrigin, path: '/' });
  } finally {
...
try {
  ...
  result = {
    ...
    passCriteria: {
      ...
      historyPreparedWithoutAuthorizationPost: true,

...
计划见 [PLAN.md](./PLAN.md)，决定记录见 [decisions.tsv](./decisions.tsv)。分支 `cursor/api-redesign-db-plan-2dfd`，基线 `main` 提交 `f0b0e68`。
...
- 调研：两个 grok 4.6 只读子代理分别整理 21 份 ADR 的 API 约束和现有 `internal/web` 路由、已采用契约、旧目录端点。
...
- ADR 正文没有改动。采用方案后 ADR-0009 的替代关系、ADR 索引对 ADR-0021 的收录都还没有写入。

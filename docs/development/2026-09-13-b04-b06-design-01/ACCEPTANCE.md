...
状态：PLAN_ONLY / ALL_RUNTIME_CASES_NOT_RUN。本轮检查的是声明和规则之间的对应关系；下表全是后续实施验收，未启动数据库、服务、浏览器、容器或模型。不能将行数/静态检查通过代作业务通过。测试时使用实施者自有临时数据库、受控HTTP/TLS记录端点和明确故障点；真实外部验收另获授权。

...
| 项目 | 实际值 |
| --- | --- |
| 开始与结束时间，含时区 | 开始 2026-09-14 11:57 UTC。LIVE-09/10 仍在等自然刷新，尚未结束 |
...
| 浏览器版本、桌面与移动视口 | VM Chrome。桌面已观察。移动 390×844 留到 LIVE-10 |
| 两个进程 UID、PID、启动时间与停止安排 | Web 与 coordinator 的 PID 以状态目录为准。环境 `:8080` 不纳入 |
| 用户令牌过期已启用、初始 expires_at、refresh_due_at | `access_expires_at` `2026-09-15T01:26:40Z`；`refresh_due_at` `2026-09-15T01:26:10Z`。禁止改库到期时间 |
| 替身、时间修改和请求重放 | 未使用 |
...
- B02.6 本轮未通过。B02 整体仍为 IN_PROGRESS。LIVE-09/10 仍在等自然刷新。

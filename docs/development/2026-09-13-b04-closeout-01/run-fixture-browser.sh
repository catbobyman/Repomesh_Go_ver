...
mode=${1:?usage: run-fixture-browser.sh b04|b03-normal|b03-empty evidence-directory}
evidence=${2:?usage: run-fixture-browser.sh b04|b03-normal|b03-empty evidence-directory}
case "$mode" in b04|b03-normal|b03-empty) ;; *) echo "mode must be b04, b03-normal or b03-empty" >&2; exit 2 ;; esac
if [[ -e "$evidence" ]]; then echo "evidence directory already exists" >&2; exit 2; fi

repo=$(cd "$(dirname "$0")/../../.." && pwd)
here=$(cd "$(dirname "$0")" && pwd)
postgres_bin=/usr/lib/postgresql/17/bin
runtime=$(mktemp -d "/tmp/repomesh-${mode}-browser.XXXXXX")
data="$runtime/data"
...
case "$mode" in
  ...
  b03-empty)
    empty=1
    script="$runtime/b03-browser-acceptance.mjs"
    ;;
  b03-normal)
    script="$runtime/b03-browser-acceptance.mjs"
    ;;
...
if [[ "$mode" == b03-normal || "$mode" == b03-empty ]]; then
  python3 - "$repo/docs/development/2026-09-12-b03-integration-01/browser-acceptance.mjs" "$script" <<'PY'
from pathlib import Path
import sys
source, target = Path(sys.argv[1]), Path(sys.argv[2])
text = source.read_text(encoding="utf-8")
text = text.replace(
    "/tmp/repomesh-b02-browser/node_modules/playwright",
    "/tmp/repomesh-b04-browser/node_modules/playwright-core",
)
text = text.replace(
    "/tmp/repomesh-b02-browser/browsers/chromium-1243/chrome-linux64/chrome",
...
REPOMESH_TEST_DATABASE_URL="$database_url" REPOMESH_B03_BROWSER_TEST=1 REPOMESH_B03_BROWSER_ASSETS="$repo/web/dist" REPOMESH_B03_BROWSER_MANIFEST_DIR="$manifest_dir" REPOMESH_B03_BROWSER_DURATION="$duration" REPOMESH_B03_BROWSER_EMPTY_CATALOG="$empty" \
...
set +e
SCREENSHOT_DIR="$evidence/screenshots" ARTIFACT_DIR="/opt/cursor/artifacts" \
...
python3 - "$evidence/browser-result.json" "$evidence/network-whitelist.json" "$mode" <<'PY'
